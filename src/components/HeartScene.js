...
const Cake = () => {
  ...
  useEffect(() => {
    if (cakeRef.current) {
      cakeRef.current.rotation.y = Math.PI / 6; // Góc xoay ban đầu
    }
  }, []);

  useFrame(() => {
    if (cakeRef.current) {
      cakeRef.current.rotation.y += 0.005; // Xoay chậm
    }
  });

  return (
    <group ref={cakeRef} position={[0, 0, 0]} scale={[1.5, 1.5, 1.5]}>
      {/* Thân bánh sinh nhật */}
      <mesh position={[0, 0.5, 0]}>
        <cylinderGeometry args={[1, 1, 1, 32]} />
        <meshStandardMaterial color="#ffcc99" roughness={0.5} metalness={0.3} />
      </mesh>

      {/* Kem phủ trên bánh */}
      <mesh position={[0, 1.1, 0]}>
        <torusGeometry args={[0.9, 0.2, 16, 100]} />
      ...
      <mesh position={[0, 1.8, 0]}>
        ...
        <meshStandardMaterial color="#ffdd44" emissive="#ff6600" />
      </mesh>
    </group>
  );
};
